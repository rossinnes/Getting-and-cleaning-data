# Question You should create one R script called run_analysis.R that does the following: 
# Merges the training and the test sets to create one data set.
# so this script merges data from a number of .txt files and produces 
...
if (!("reshape2" %in% rownames(installed.packages())) ) {
  ...
  activity_labels <- read.table("/Users/rossinnes/Documents/RDirectory/activity_labels.txt",col.names=c("activity_id","activity_name"))
  
  # Read the dataframe's column names
  features <- read.table("/Users/rossinnes/Documents/RDirectory/features.txt")
  feature_names <-  features[,2]
  
  # Read the test data and label the dataframe's columns
  testdata <- read.table("/Users/rossinnes/Documents/RDirectory/test/X_test.txt")
  colnames(testdata) <- feature_names
  
  # Read the training data and label the dataframe's columns
  traindata <- read.table("/Users/rossinnes/Documents/RDirectory/train/X_train.txt")
  colnames(traindata) <- feature_names
  
  # Read the ids of the test subjects and label the the dataframe's columns
  test_subject_id <- read.table("/Users/rossinnes/Documents/RDirectory/test/subject_test.txt")
  colnames(test_subject_id) <- "subject_id"
  
  # Read the activity id's of the test data and label the the dataframe's columns
  test_activity_id <- read.table("/Users/rossinnes/Documents/RDirectory/test/y_test.txt")
  colnames(test_activity_id) <- "activity_id"
  
  # Read the ids of the test subjects and label the the dataframe's columns
  train_subject_id <- read.table("/Users/rossinnes/Documents/RDirectory/train/subject_train.txt")
  colnames(train_subject_id) <- "subject_id"
  
  # Read the activity id's of the training data and label 
  # the dataframe's columns
  train_activity_id <- read.table("/Users/rossinnes/Documents/RDirectory/train/y_train.txt")
  ...
  write.table(mean_data,"/Users/rossinnes/Documents/RDirectory/tidy_movement_data.txt", row.name=FALSE)
